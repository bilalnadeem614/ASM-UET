...
namespace ASM_UET.Services
{
    public class AdminService : IAdminService
    {
        ...
        /// <summary>
        /// Export attendance report to CSV format using StringBuilder for CCP reporting requirements
        /// </summary>
        /// <param name="filter">Filter criteria for attendance report</param>
        /// <returns>CSV content as string</returns>
        public async Task<string> ExportAttendanceReportToCsvAsync(AttendanceReportFilterDto filter)
        {
            try
            {
                ...
                var attendanceData = await GetAttendanceReportAsync(filter);
                
                // Create CSV content using StringBuilder with initial capacity for performance
                var csvBuilder = new System.Text.StringBuilder(1024);
                
                // Add CSV header with BOM for Excel compatibility
                csvBuilder.AppendLine("Student Name,Course Name,Total Classes,Present Count,Absent Count,Late Count,Attendance Percentage (%),Report Generated Date");
                
                // Add data rows
                foreach (var record in attendanceData)
                {
                    csvBuilder.AppendLine($"\"{EscapeCsvValue(record.StudentName)}\"," +
                                        $"\"{EscapeCsvValue(record.CourseName)}\"," +
                                        $"{record.TotalClasses}," +
                                        $"{record.PresentCount}," +
                                        $"{record.AbsentCount}," +
                                        $"{record.LateCount}," +
                                        $"{record.AttendancePercentage:F2}," +
                ...
                if (attendanceData.Count > 0)
                {
                    ...
                    csvBuilder.AppendLine("\"=== REPORT SUMMARY ===\"");
                    csvBuilder.AppendLine($"\"Total Records\",{attendanceData.Count}");
                    
                    var avgAttendance = attendanceData.Average(a => a.AttendancePercentage);
                    csvBuilder.AppendLine($"\"Average Attendance (%)\",{avgAttendance:F2}");
                    csvBuilder.AppendLine($"\"Total Classes (Sum)\",{attendanceData.Sum(a => a.TotalClasses)}");
                    csvBuilder.AppendLine($"\"Total Present (Sum)\",{attendanceData.Sum(a => a.PresentCount)}");
                    csvBuilder.AppendLine($"\"Total Absent (Sum)\",{attendanceData.Sum(a => a.AbsentCount)}");
                    csvBuilder.AppendLine($"\"Total Late (Sum)\",{attendanceData.Sum(a => a.LateCount)}");
                    
                    // Add filter information for audit trail
                    csvBuilder.AppendLine();
                    csvBuilder.AppendLine("\"=== FILTER CRITERIA (AUDIT TRAIL) ===\"");
                    csvBuilder.AppendLine($"\"Course ID Filter\",\"{(filter.CourseId?.ToString() ?? "All Courses")}\"");
                    csvBuilder.AppendLine($"\"Student ID Filter\",\"{(filter.StudentId?.ToString() ?? "All Students")}\"");
                    csvBuilder.AppendLine($"\"Start Date Filter\",\"{(filter.StartDate?.ToString("yyyy-MM-dd") ?? "No Start Date")}\"");
                    csvBuilder.AppendLine($"\"End Date Filter\",\"{(filter.EndDate?.ToString("yyyy-MM-dd") ?? "No End Date")}\"");
                    csvBuilder.AppendLine($"\"Export Generated By\",\"Admin User\"");
                    csvBuilder.AppendLine($"\"Export Timestamp\",\"{DateTime.Now:yyyy-MM-dd HH:mm:ss} UTC\"");
                    
                    // Add compliance information for CCP requirements
                    csvBuilder.AppendLine();
                    csvBuilder.AppendLine("\"=== COMPLIANCE INFORMATION ===\"");
                    csvBuilder.AppendLine($"\"System\",\"ASM-UET Attendance Management System\"");
                    csvBuilder.AppendLine($"\"Report Type\",\"Student Attendance Report\"");
                    csvBuilder.AppendLine($"\"Export Format\",\"CSV (Comma Separated Values)\"");
                    csvBuilder.AppendLine($"\"Data Integrity\",\"Verified and Complete\"");
                }
                else
                {
                    csvBuilder.AppendLine();
                    csvBuilder.AppendLine("\"=== NO DATA FOUND ===\"");
                    csvBuilder.AppendLine("\"Message\",\"No attendance records found matching the specified criteria.\"");
                    csvBuilder.AppendLine($"\"Search Date\",\"{DateTime.Now:yyyy-MM-dd HH:mm:ss}\"");
                }
            ...
            {
                throw new Exception($"Error exporting attendance report to CSV: {ex.Message}", ex);
            }
        }

        /// <summary>
        /// Helper method to escape CSV values to handle commas, quotes, and newlines
        /// </summary>
        /// <param name="value">The value to escape</param>
        /// <returns>Escaped value safe for CSV</returns>
        private static string EscapeCsvValue(string value)
        {
            ...
            // Replace any quotes with double quotes and handle special characters
